...
public class ArchonPlayer extends BasePlayer {
	 ...
	 */
	public void spawnUnitAndTransferFlux(RobotType unitType, double fractionOfMaxFlux) {
		while (true) {
			try {
				while (myRC.isMovementActive()) {
					runAtEndOfTurn();
	...
	/**
	 * ATTEMPTS to spawns a unit and transfers flux to it. Causes this archon
	 * to wait (yielding) until it has enough flux to transfer to a unit. Does
	 * NOT cause the archon to wait until it has enough flux to spawn a unit.
	 */
	...
	/**
	 * Overloaded attemptSpawnUnit function that allows for custom flux transfer.
	 */
	public void attemptSpawnUnitAndTransferFlux(RobotType unitType, double fractionOfMaxFlux) {
		try {
			while (myRC.isMovementActive()) {
				super.runAtEndOfTurn();
			}
